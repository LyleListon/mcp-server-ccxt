...
class EnhancedArbitrageBot:
    ...
    async def _analyze_opportunity(self, opportunity: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        try:
            ...
            if not profit_result.get('is_profitable', False):
                logger.debug(f"Opportunity filtered: not profitable")
                return None
            
            if profit_result.get('net_profit_percentage', 0) < self.min_profit_threshold:
                logger.debug(f"Opportunity filtered: profit below threshold")
                return None
            
            if risk_result.get('risk_score', 100) > self.max_risk_score:
                logger.debug(f"Opportunity filtered: risk too high")
                return None
