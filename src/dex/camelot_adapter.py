...
Camelot is a native Arbitrum DEX with unique features and often less efficient pricing.
"""
...
class CamelotAdapter(BaseDEX):
    """Camelot DEX adapter for Arbitrum arbitrage."""

    def __init__(self, config: Dict[str, Any]):
        """Initialize Camelot adapter."""
        super().__init__("camelot", config)

        # Camelot API endpoints (Arbitrum) - with API key
        self.base_url = "https://api.camelot.exchange"
        self.subgraph_url = "https://gateway-arbitrum.network.thegraph.com/api/fc2235999cc4344e7c8722107c9c0bd6/subgraphs/name/camelotlabs/camelot-amm"
        # Fallback to public endpoint
        self.subgraph_url_fallback = "https://api.thegraph.com/subgraphs/name/camelotlabs/camelot-amm"
        ...
        logger.info(f"Camelot adapter initialized for {self.name}")

    async def connect(self) -> bool:
        """Connect to Camelot API."""
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    if 'data' in data and 'pairs' in data['data']:
                        ...
                        logger.info("✅ Connected to Camelot (Arbitrum)")
                        return True

            logger.error("Failed to connect to Camelot subgraph")
            return False

        except Exception as e:
            logger.error(f"Error connecting to Camelot: {e}")
            return False

    async def get_pairs(self) -> List[Dict[str, Any]]:
        """Get available trading pairs from Camelot."""
        try:
            pairs = []

            # Create pairs from common Arbitrum tokens
            common_tokens = ['ETH', 'WETH', 'USDC', 'USDT', 'DAI', 'WBTC', 'ARB']
                    ...
                    try:
                        ...
                        if price and price > 0:
                            pair = {
                                ...
                                'dex': self.name,
                                'price': price,
                                'liquidity': 500000,  # Smaller DEX, lower liquidity
                                'volume_24h_usd': 2000000,  # ~$2M daily volume
            ...
            logger.info(f"Fetched {len(pairs)} pairs from Camelot")
            return pairs

        except Exception as e:
            logger.error(f"Error fetching pairs from Camelot: {e}")
            return []

    async def get_price(self, base_token: str, quote_token: str) -> Optional[float]:
        """Get current price for a token pair using Camelot subgraph."""
        try:
            ...
            # Query Camelot subgraph for pair data
            query = f"""
            {{
                pairs(
                    where: {{
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.warning(f"Camelot query failed: HTTP {response.status}")
                    return None

        except Exception as e:
            logger.error(f"Error getting Camelot price for {base_token}/{quote_token}: {e}")
            return None

    async def get_liquidity(self, base_token: str, quote_token: str) -> Optional[float]:
        """Get liquidity for a token pair."""
        return 500000.0  # $500K typical liquidity for smaller DEX
    ...
    async def get_quote(self, base_token: str, quote_token: str, amount: float) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'price': price,
                'slippage_estimate': 0.3,  # Higher slippage for smaller DEX
                'gas_estimate': 200000,  # Arbitrum gas estimate
    ...
    async def disconnect(self) -> None:
        """Disconnect from Camelot API."""
        if self.session:
            await self.session.close()
            self.session = None

        self.connected = False
        logger.info("Disconnected from Camelot")
