...
import { PennyTrade, GasTracker, ArbitrageOpportunity, MemoryQuery, PerformanceStats } from './types';

export class DexMindMemory {
  private db: sqlite3.Database;

  constructor(dbPath: string = './dexmind.db') {
  ...
  async storePennyTrade(trade: PennyTrade): Promise<void> {
  ...
  async getGreenTrades(limit: number = 100): Promise<PennyTrade[]> {
    return new Promise((resolve, reject) => {
      this.db.all(`
      ...
      `, [limit], (err, rows: any[]) => {
        if (err) reject(err);
        else resolve(rows.map(this.rowToPennyTrade));
      });
    });
  }
  ...
  async storeGasData(gasData: GasTracker): Promise<void> {
    return new Promise((resolve, reject) => {
      const stmt = this.db.prepare(`
  ...
  private rowToPennyTrade(row: any): PennyTrade {
    return {
      id: row.id,
      tokenA: row.token_a,
      tokenB: row.token_b,
